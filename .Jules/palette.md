...
## 2025-02-13 - Dismiss Software Keyboard on Search Action
**Learning:** In Jetpack Compose, the software keyboard does not dismiss automatically when a user triggers an IME action like "Search" via `KeyboardOptions`. This leaves the keyboard obscuring search results.
**Action:** Always provide explicit `KeyboardActions(onSearch = { focusManager.clearFocus() })` utilizing `LocalFocusManager.current` to ensure the keyboard is dismissed, providing a smoother and more expected user experience.
