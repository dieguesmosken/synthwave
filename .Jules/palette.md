...
**Learning:** Users often double-click login buttons out of habit or impatience when asynchronous auth flows (like Google Sign-In) take a second to process. This can lead to concurrent auth requests, race conditions, or unhandled exceptions.
**Action:** Always add an explicit visual loading state (e.g., `CircularProgressIndicator`) and disable the button while loading to prevent multiple submissions and improve perceived performance during async operations.
